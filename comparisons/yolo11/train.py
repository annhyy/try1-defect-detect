...
def main() -> None:
    ...
    model_source = "yolo11n.pt" if args.pretrained else "yolo11n.yaml"
    print(f"模型初始化：{model_source}；{'使用 COCO 预训练权重' if args.pretrained else '从零随机初始化'}")
    model = YOLO(model_source)
    result = model.train(data=args.data, epochs=args.epochs, imgsz=args.img_size, batch=args.batch_size, device=args.device, seed=args.seed, pretrained=args.pretrained, project=str(ROOT / "comparisons" / "yolo11" / "runs"), name="apspc", exist_ok=True)
