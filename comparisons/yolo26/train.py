...
def main() -> None:
    ...
    model = YOLO("yolo26n.pt" if args.pretrained else "yolo26n.yaml")
    result = model.train(data=args.data, epochs=args.epochs, imgsz=args.img_size, batch=args.batch_size, device=args.device, seed=args.seed, project=str(ROOT / "comparisons" / "yolo26" / "runs"), name="apspc", exist_ok=True)
