...
最后一个 stride=1 卷积中加入通道二次积分的 Dit-CNN，以及用 2x2 空间树突运算
替换一个或两个 stride=1 卷积的 DendriticConv-R1/R2。DendriticConv 中的
`patch_items=4` 来自 2x2 空间窗口，`branches=4` 是每个输出通道的并行树突分支，
二者不是同一个概念；外部基线为 YOLO11n、
...
- DNM、Dit-CNN 和空间树突卷积的 `metrics.csv`：train/val total、objectness、
  box、class loss，以及 P/R/mAP。
