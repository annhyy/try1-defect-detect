...
`alfoil_dnm/data.py` 只读取这种 YOLO 目录和 `data.yaml`。正式的受控对比实验使用等比例 Letterbox 缓存后的 `datasets/apspc_yolo_letterbox640/data.yaml`；未来用铝箔产线数据时，也应先整理为同样的 YOLO 目录结构，并为各对比模型使用同一份预处理结果。
