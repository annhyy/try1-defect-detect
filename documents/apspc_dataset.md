...
`alfoil_dnm/data.py` 只读取这种 YOLO 目录和 `data.yaml`。因此训练命令里 `--data` 指向哪个 YAML，就使用哪一套数据。
...
### `demo_alfoil`：仓库内置的合成运行示例

`demo_alfoil` 由 `alfoil_dnm/make_demo.py` 绘制生成，包含 150 张 256 x 256 的合成图和 YOLO TXT 标签：120 张训练、30 张验证，类别为 `hole`、`scratch`、`pit`、`stain`、`insect`。它不是下载的真实数据，也不能用于报告产线精度。此前得到的 `runs/demo_dnm/best.pt` 就是用这一数据训练的。

该数据会提交到 Git，因此新用户克隆仓库、安装依赖后可以直接运行 Demo 命令。

### `datasets/apspc_yolo`：本地真实 APSPC 转换数据

该目录由 APSPC XML 转换而来，包含真实铝型材图片和 10 类缺陷标签。真实训练命令使用的就是 `datasets/apspc_yolo/data.yaml`，而不是 `demo_alfoil/data.yaml`。截至本文档更新时，APSPC 已完成转换，但尚未产生有效的完整训练权重；此前交互会话对前台长训练有 60 秒限制。
