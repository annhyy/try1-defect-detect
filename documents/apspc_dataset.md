...
> 当前主任务已恢复为 **目标检测**。所有模型读取 APSPC 的 YOLO 框，输出缺陷位置、
> 类别和置信度；旧 X-SDD 七分类及 `runs1/` 只作历史记录。
...
4. 按本表 ID 将字符串类别转为数字，计算并写入 YOLO 行：
   ...
   例如 `budaodian`（ID 1）的像素框 `(1, 835, 2560, 1313)` 在 2560 x 1920 图片上会写成近似：`1 0.500195 0.559375 0.999609 0.248958`。
...
`alfoil_dnm/data.py` 只读取这种 YOLO 目录和 `data.yaml`。正式的受控对比实验使用等比例 Letterbox 缓存后的 `datasets/apspc_yolo_letterbox640/data.yaml`；新结果写入 `run2/controlled/`，不覆盖 `runs/` 和 `runs1/` 的旧结果。未来用铝箔产线数据时，也应先整理为同样的 YOLO 目录结构，并为各对比模型使用同一份预处理结果。
...
### 训练缓存：等比例 Letterbox 640

APSPC 原图通常为 2560 x 1920。若在每个 epoch 中直接读取这些原图，训练器会重复执行 JPEG 解码与缩放，CPU 和磁盘会成为瓶颈。因此 `cache_letterbox.py` 在 `datasets/apspc_yolo_letterbox640` 新建独立缓存数据集：原图按比例缩放为 640 x 480，并在上下各填充 80 像素的灰色边，得到 640 x 640 图像。它不是拉伸变形，4:3 的长宽比例和缺陷形状保持不变。

每个缺陷框也会从原图坐标同步映射到带填充的 640 x 640 坐标系；因此缓存目录下的标签与图像严格对应，不能将原 `apspc_yolo/labels` 与缓存图像混用。缓存仅是本地加速数据，不提交到 Git。手写数字 MNIST 常用 28 x 28，是低分辨率分类基准；铝箔缺陷检测需要保留细小纹理，640 是 YOLO 检测中常用的输入尺度。
...
该目录由 APSPC XML 转换而来，包含真实铝型材图片和 10 类缺陷标签。实际训练使用其等比例 Letterbox 缓存副本 `datasets/apspc_yolo_letterbox640`，以避免每个 epoch 重复解码、缩放 2560 × 1920 原图；缓存图像与标签已同步变换，不能与原始目录的标签混用。
