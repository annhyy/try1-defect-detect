...
当前真实训练数据为 **APSPC（Aluminum Profile Surface Detection Database）**：1,885 张铝型材图像、10 个类别、3,143 个 Pascal VOC XML 标注框。它用于预训练与方法验证；最终部署前必须用铝箔产线图像对破洞、蚊虫等类别进行微调。
...
若已解压 APSPC 原始压缩包到 `datasets/APSPC1`、`datasets/APSPC2` 和 `datasets/APSPC-Annotations`：
...
源码、文档和配置提交到仓库；数据集、权重、运行日志、IDE 缓存由 `.gitignore` 排除。每次真实实验完成后，在 `documents/apspc_dataset.md` 中记录数据版本、超参数、硬件、mAP、各类召回率和失败样例，再提交对应代码与记录。
